...
void loop(){
  ...
  if(Serial.available() <= 0 && flag == true) { 
    ...
    if(finalMessage[0] == '@'){ // сообщение адресовано для всех
      ...
      if(finalMessage[0] == '2' || isAll){
      finalMessage[0] = ' ';
          Wire.beginTransmission(2); // начать передачу для DEVICE_ID = 2
          Wire.write(finalMessage, messageSize); //отправить сообщение по шине
      Wire.endTransmission();   // закончить передачу
      
      if(!isAll){
          Serial.write(finalMessage, messageSize); // сообщение данного девайса распечатать в Serial
      }
      
      }

      if(finalMessage[0] == '3' || isAll){
      finalMessage[0] = ' ';
      Wire.beginTransmission(3);  // начать передачу для DEVICE_ID = 3
    ...
    /*после отправки сообщения присвоить начальные зна-я: */
      messageSize=0;
